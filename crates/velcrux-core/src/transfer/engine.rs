...
use crate::protocol::message::{
    Commit as CommitMsg, Committed as CommittedMsg, Verify as VerifyMsg,
    VerifyResult as VerifyResultMsg,
};
use crate::storage::{FileMeta, LocalFilesystemBackend, StagingWriter, StorageBackend, VPath};
use crate::transport::{BiRecvStream, BiSendStream, Connection, UniRecvStream};
...
pub async fn client_upload(
    ...
    // Bounded channel between chunker task and writer task.
...
pub async fn server_upload_session(
    ...
    loop {
        ...
        // Read the declared payload length. We trust the declared length
        // because it is bounded by DATA_MAX_CHUNK_LEN (4 MiB).
